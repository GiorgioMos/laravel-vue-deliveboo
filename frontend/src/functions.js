...
export default {
    ...
    minBtn.forEach((btn) => {
      if (btn) {
        if (valore == 0) {
          ...
          btn.style.color = "#060113";
          btn.style.border = "solid 1px #060113";
          btn.style.cursor = "initial";
        } else {
          if (btn.classList.contains("visibility")) {
            btn.classList.remove("visibility");
            btn.style.color = "#ff9900";
            btn.style.border = "solid 1px #ff9900";
  ...
  cartAddElement: function (product) {
    ...
    if (
      ...
      product.restaurant_id == localStorage.getItem("restaurant_id")
    ) {
